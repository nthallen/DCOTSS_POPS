#! /bin/bash
# Install script for DPOPS
function nl_error {
  echo "dpops_matlab_setup: $*" >&2
  exit 1
}

scriptname=dpops_matlab_setup
ofile=$scriptname.m
OS=`uname -s`
use_cygpath=no
case "$OS" in
...
git clone ${method}github.com/nthallen/DCOTSS_POPS.git DPOPS
wrap_path $PWD/DPOPS/eng DPOPS_eng_wrap_path
cat >$ofile <<EOF
...
addpath('$DPOPS_eng_wrap_path');
savepath;
delete $ofile
fprintf(1,'DPOPS Setup complete\n');
pause(2);
quit
EOF

cd ..
cp SW/DPOPS/eng/getrun Data/DPOPS/
wrap_path $PWD/Data/DPOPS/RAW RAW_wrap_path
